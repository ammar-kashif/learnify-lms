  ...
  const testAuthAndInsert = async () => {
    ...
    try {
      ...
      if (user) {
        ...
        addResult(`📋 Attempting to insert profile: ${JSON.stringify(testProfile, null, 2)}`);
        ...
        if (insertError) {
          ...
          addResult(`✅ Profile insertion successful: ${JSON.stringify(insertData, null, 2)}`);
        }
      } else {
      ...
      if (user) {
        ...
        if (selectError && selectError.code === 'PGRST116') {
          addResult('ℹ️ No existing profile found (this is expected for new users)');
        } else if (selectError) {
          addResult(`❌ Profile select error: ${selectError.message}`);
        } else if (existingProfile) {
          addResult(`✅ Existing profile found: ${JSON.stringify(existingProfile, null, 2)}`);
        }
      }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-8">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold text-gray-900 mb-6">Authentication & User Insertion Test</h1>
        
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
          <h2 className="text-xl font-semibold mb-4">Test Authentication & Database Insertion</h2>
          <p className="text-gray-600 mb-4">
            This page will test your authentication state and try to insert a user profile to identify the RLS issue.
          </p>
